...
[Hermes Agent](https://hermes-agent.nousresearch.com/) 是一个运行在您服务器上的复杂自主智能体，可通过终端或消息应用访问，它会记住所学内容，运行时间越长能力越强。

Hermes WebUI 是一个轻量级的暗色主题 Web 应用界面，用于在浏览器中访问 [Hermes Agent](https://hermes-agent.nousresearch.com/)。
与 CLI 体验完全对等 — 您在终端中可以执行的所有操作，
都可以在此界面中完成。无需构建步骤，无需框架，无需打包器。只有 Python
和原生 JavaScript。
...
1. **双击** `start.bat` 或在 PowerShell 中运行 `.\start.ps1`
2. **打开**浏览器访问 `http://127.0.0.1:8787`
3. **开始**与 Hermes 聊天！
...
**详细 Windows 指南请参见 [QUICKSTART-WINDOWS.md](QUICKSTART-WINDOWS.md)。**
...
大多数 AI 工具在每个会话后都会重置。它们不知道您是谁，您在工作什么，或者
您的项目遵循什么约定。每次您都需要重新解释。

Hermes 跨会话保留上下文，在您离线时运行计划任务，并随着运行时间越长变得
对您的环境越了解。它使用您现有的 Hermes agent 设置，
您的现有模型，并且无需额外配置即可启动。

它与其他智能体工具的区别：

- **持久化记忆** — 用户档案、智能体笔记，以及保存可重用
  流程的技能系统；Hermes 学习您的环境，无需重新学习
- **自托管调度** — 在您离线时触发的 cron 作业，并将结果传递到
  Telegram、Discord、Slack、Signal、电子邮件等
- **10+ 消息平台** — 终端中可用的同一智能体可通过手机访问
- **自我改进的技能** — Hermes 从经验中自动编写并保存自己的技能；
  无需浏览市场，无需安装插件
- **提供商无关** — OpenAI、Anthropic、Google、DeepSeek、OpenRouter 等
- **协调其他智能体** — 可为繁重的编码任务生成 Claude Code 或 Codex 并将
  结果带回自己的记忆
- **自托管** — 您的对话、您的记忆、您的硬件
